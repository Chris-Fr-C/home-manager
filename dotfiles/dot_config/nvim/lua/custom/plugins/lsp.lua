...
vim.api.nvim_create_autocmd('LspAttach', {
  ...
  callback = function(event)
    ...
    map(containers.lsp.key .. 'a', vim.lsp.buf.code_action, 'Code [a]ction', { 'n', 'x' })
    ...
    if client and client:supports_method('textDocument/inlayHint', event.buf) then
      map(containers.config.key .. 'h', function() vim.lsp.inlay_hint.enable(not vim.lsp.inlay_hint.is_enabled { bufnr = event.buf }) end, 'Toggle Inlay [h]ints')
    end
  end,
})
